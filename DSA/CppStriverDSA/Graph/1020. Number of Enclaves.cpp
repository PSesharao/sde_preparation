...
public:
    ...
    void dfs( int x , int y , vector<vector<int>>& board , 
        ...
        for(int i=0 ; i<4 ; i++ ){
            ...
            if( next_x >=0 and next_x < rows and next_y >=0 and next_y < cols ){
                ...
                if( !visited[next_x][next_y] and board[next_x][next_y] == 1  )
    ...
    int numEnclaves(vector<vector<int>>& board) {
        ...
        for(int y=0 ; y<cols ; y++ )
            if( board[0][y] == 1 and !visited[0][y] )
                dfs(0 , y , board , visited  ) ; 

        // last row
        for(int y=0 ; y<cols ; y++ )
            if( board[rows-1][y] == 1 and !visited[rows-1][y] )
                dfs(rows-1 , y , board , visited  ) ;             

        // first col
        for(int x=0 ; x<rows ; x++ )
            if( board[x][0] == 1 and !visited[x][0] )
                dfs(x , 0 , board , visited  ) ;             

        // last col 
        for(int x=0 ; x<rows ; x++ )
            if( board[x][cols-1] == 1 and !visited[x][cols-1] )
